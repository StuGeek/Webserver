...
日志单元在`log.cpp`和`log.h`文件中实现，使用了单例模式，有同步和异步两种写入方式，当程序的某处使用日志写入方法时，首先获取单例中的是否打开日志变量，判断是否开启了日志功能，没有则不写入日志信息，有则使用可变参数将内容按照格式传递进日志方法中，日志方法经过格式处理和拼接后将内容写入相应的日志文件，如果是异步写入，会先将日志内容的字符串放进一个阻塞队列中，由日志线程异步将其写入日志文件。
