...
private:
    ...
    void req_func(void *arg);        // 每个任务请求执行的函数
...
template <typename T>
void ThreadPool<T>::req_func(void *arg) {
    // 向每个线程传入的参数是线程池
    ThreadPool *pool = (ThreadPool *)arg;
    // 如果is_end不为真，线程池中的线程进行请求处理，否则结束线程池
    while (!pool->is_end) {
        // 请求队列中的请求数减一
        sem_wait(&pool->sem_req_num);
        // 保护队列
        pthread_mutex_lock(&pool->mutex_req_que);
        
        // 取队首的请求进行处理
        T *req = pool->req_que.front();
        pool->req_que.pop();
        Mysql_conn_RAII mysql_coon(&req->mysql, mysql_connpool);
        // 线程池中的线程执行的处理函数，需要在req的具体类中实现方法
        req->process();
        
        pthread_mutex_unlock(&pool->mutex_req_que);
    }
}
